    ...
    return (
        <div ref={wrapperRef} className='skillCard p-3 flex flex-col gap-2 rounded-sm relative cursor-pointer' onClick={() => setSetting(!setting)}>
            ...
            <div className='skillSettingsWrapper flex gap-0.5 absolute w-full h-full top-0 left-0'>
                <div  className='w-1/2 flexCenter cursor-pointer' title='Edit' onClick={() => {setForm(true); setSkillObj(skill);  setOldName(skill.imageLink || ''); setFolder('frontend')}}>
            ...
            <div className='flex gap-1 items-center'>
                {skill.imageLink &&
                <Image
                    src={`https://res.cloudinary.com/dswmp2omq/image/upload/v1750627282/portfolio/skills/frontend/${skill.imageLink}`}
                    alt='Image'
                    width={20}
                    height={20}
                    loading='lazy'
